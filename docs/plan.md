...
| **0.5.2 patch** | **done — zigbee 0.5.2** ✅ | First multi-platform binary release. `build.zig` refactored to compile libsecp256k1 from C sources directly (`-DENABLE_MODULE_RECOVERY=1`, three .c files); replaces the host-built `vendor/secp256k1/build/lib/libsecp256k1.a` archive that prevented cross-compilation. CI workflow at `.github/workflows/release.yml` builds six binaries on every `v*.*.*` tag push: `x86_64-linux-musl`, `aarch64-linux-musl`, `aarch64-linux-gnu`, `arm-linux-musleabihf` (Pi Zero W), `x86_64-macos`, `aarch64-macos`. Sizes 1.9–6.8 MB ReleaseSafe. Small `@fieldParentPtr` `@alignCast` fix in `src/store.zig` for 32-bit ARM. No protocol or runtime-behaviour change. Tests still 113/113. See [`release-notes/0.5.2.md`](release-notes/0.5.2.md). |
|  | | |
...
| 0.7a — ARM Linux release matrix | **mostly shipped in 0.5.2** ✅ | Cross-compile + statically-linked binaries for `x86_64-linux-musl`, `aarch64-linux-musl`, `aarch64-linux-gnu`, `arm-linux-musleabihf` (Pi Zero W), `x86_64-macos`, `aarch64-macos` shipping on every `v*.*.*` tag push via `.github/workflows/release.yml` since 0.5.2. `vendor/secp256k1` compiled from C sources by `build.zig` (no external toolchain needed). **Remaining:** hardware-validate on actual Pi Zero W; document recommended per-target settings (paired with X1 below). ~2-3 days. |
...
| X3 — Continuous cross-compilation in CI | **introduced in 0.5.2** (tag-push only); formalisation = move to every-commit | GitHub Actions matrix at `.github/workflows/release.yml` rebuilds `vendor/secp256k1` for six targets on every `v*.*.*` tag push (x86_64/aarch64/armv7 musl + aarch64-gnu + macOS x86_64/arm64). To catch Linux-isms earlier, a follow-up adds the same matrix to a `push:` workflow on every commit (cheap — only a `zig build` per target, no test). |
...
| X5 — Static-link path | **shipped in 0.5.2** ✅ | The `*-linux-musl` targets in the release matrix produce statically-linked single-file binaries against musl + the in-tree-built libsecp256k1. `scp zigbee-linux-arm64-musl <device>:/usr/local/bin/zigbee` deploys with no runtime dependencies on Alpine / OpenWRT / busybox-based ARM Linux. |
|  | | |
